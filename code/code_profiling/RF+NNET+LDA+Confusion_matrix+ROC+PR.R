...
write.csv(summary_all, "/results/overall_metrics_RF_LDA_NNET_cli.csv", row.names = FALSE)
write.csv(metrics_class_all, "/results/class_metrics_RF_LDA_NNET.csv_cli", row.names = FALSE)
write.csv(as.data.frame(rf_res$conf_mat$table), "/results/confusion_matrix_RF_cli.csv", row.names = FALSE)
write.csv(as.data.frame(lda_res$conf_mat$table), "/results/confusion_matrix_LDA_cli.csv", row.names = FALSE)
write.csv(as.data.frame(nnet_res$conf_mat$table), "/results/confusion_matrix_NNET_cli.csv", row.names = FALSE)
write.csv(roc_metrics_all, "/results/ROC_AUC_values_RF_LDA_NNET_cli.csv", row.names = FALSE)
write.csv(pr_metrics_all, "/results/PR_AUPRC_values_RF_LDA_NNET_cli.csv", row.names = FALSE)
write.csv(auc_auprc_all, "/results/ROC_AUC_PR_AUPRC_summary_RF_LDA_NNET_cli.csv", row.names = FALSE)
...
write.csv(
  cm_all_df,
  "/results/confusion_matrix_percentage_plot_data_RF_LDA_NNET_cli.csv",
  row.names = FALSE
)
...
plot_cm_without_text <- function(cm_df, model_name, display_names) {
  ...
  ggsave(
    filename = paste0("/results/Confusion_Matrix_", model_name, "_Percentage_No_Text_cli.tiff"),
    plot = p,
    width = 4.5,
    height = 4,
    units = "in",
...
ggsave(
  filename = "/results/ROC_Class_Micro_Macro_RF_LDA_NNET_cli.tiff",
  plot = p_roc,
  width = 4.5,
  height = 4,
  units = "in",
...
ggsave(
  filename = "/results/PR_Class_Micro_Macro_RF_LDA_NNET_cli.tiff",
  plot = p_pr,
  width = 4.5,
  height = 4,
  units = "in",

...
def analyze_pressure_data(
    ...
    Args:
        ...
        window_size: Number of points in the sliding slope window
    ...
    backflush_duration = calculate_backflush_duration(
        backflush_volume_ml, backflush_flow_ml_hr
    )
...
def print_results(
    results: List[Tuple[float, float, float, float, float]], backflush_duration: float
):
    ...
    for i, (trigger_time, trigger_idx, avg_slope, start_time, end_time) in enumerate(
        results
    ):
        print(f"Trigger #{i+1}:")
...
def window_size_arg(value: str) -> int:
    """Parse and validate a sliding-window size from the command line."""
    parsed = int(value)
    if parsed < 2:
        raise argparse.ArgumentTypeError("window size must be at least 2")
    return parsed
...
Examples:
  ...
  %(prog)s --help                   # Show this help message
    ...
    data_group.add_argument("--csv", type=str, help="CSV file containing pressure data")
    data_group.add_argument(
        "--duration",
        type=float,
        default=None,
        help="Duration for simulated data (seconds, default: configuration value)",
    )
    ...
    config_group.add_argument(
        "--config",
        type=str,
        help="Load a JSON configuration file (explicit CLI options take precedence)",
    )
    config_group.add_argument(
        "--save-config",
        type=str,
    ...
    detect_group.add_argument(
        ...
        type=float,
        default=None,
        help="Slope threshold for clogging detection (pressure/s)",
    )
    detect_group.add_argument(
        "--grace", type=float, default=None, help="Grace period after start (seconds)"
    )
    detect_group.add_argument(
        "--interval",
        type=float,
        default=None,
        help="Minimum interval between triggers (seconds)",
    )
    detect_group.add_argument(
        "--window",
        type=window_size_arg,
        default=None,
        metavar="N",
        help="Sliding window size, at least 2 (default: configuration value)",
    )

    # Backflush parameters
    backflush_group = parser.add_argument_group("Backflush Parameters")
    backflush_group.add_argument(
        "--flow", type=float, default=None, help="Backflush flow rate (ml/hr)"
    )
    backflush_group.add_argument(
        "--volume", type=float, default=None, help="Backflush volume (ml)"
    )
    ...
    output_group.add_argument(
        "--plot", action="store_true", help="Save pressure and slope analysis figures"
    )
    output_group.add_argument(
        "--figures-dir",
        type=str,
...
def main():
    ...
    if args.window is not None:
        updates["sliding_window_size"] = args.window
    ...
    if not config.quiet_mode:
        ...
        print(f"Sliding Window Size: {config.sliding_window_size} points")
    ...
    requested_output = (
        args.output if args.output != default_output else config.default_output_csv
    )
    output_file = resolve_output_csv_path(
        requested_output, default_filename=default_output
    )
    try:
